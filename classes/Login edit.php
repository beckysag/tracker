...
class Login
{
    ...
    public function __construct()
    {
        ...
        // 2. login via session data (happens each time user opens a page on your php 
        //		project AFTER he has sucessfully logged in via the login form)
        // 3. login via post data, which means simply logging in via the login form. after 
        //		the user has submit his login/password successfully, his
        //    logged-in-status is written into his session data on the server. this is 
        // 		the typical behaviour of common login scripts.
    ...
    {
        // if POST data (from login form) contains non-empty user_name & non-empty user_password
        if (!empty($_POST['user_name']) && !empty($_POST['user_password'])) {

			try {
	            // create a db connection, using the constants from config/db.php (which we loaded in index.php)
	            $this->db_connection = new PDO('mysql:host=' . DB_HOST . ';dbname=' . DB_NAME, DB_USER, DB_PASS);
	            //$this->db_connection = new PDO('mysql:host=' . DB_HOST . ';port=3307;dbname=' . DB_NAME, DB_USER, DB_PASS);
				...
				$stmt->execute(array('user_name' => $this->user_name));
